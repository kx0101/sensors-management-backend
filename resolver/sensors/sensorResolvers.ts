...
export const sensorsResolvers = {
    Query: {
        ...
        getSensorByAddressAndId: async (_: unknown, { address, sensor_id }: { address: string; sensor_id: number }) => {
            try {
                return await SensorRepo.findOne({ address: address, sensor_id: sensor_id });
            } catch (err) {
                logger.error(err.message);
                throw new Error(`Failed to fetch sensor with address ${address} and sensor_id ${sensor_id}`);
            }
        },
    },
    Mutation: {
        ...
        updateStatusSensor: async (_: unknown, { _id, status }: { _id: string; status: boolean }) => {
            try {
