...
export const sensorsResolvers = {
    ...
    Mutation: {
        createSensor: async (sensorInput: SensorCreate):Promise<Sensor> =>{
            const sensor = await SensorRepo.create(sensorInput).catch((err:Error) => {
        ...
        updateSensor: async ( sensorInput: SensorUpdate ):Promise<Sensor> =>{
            ...
            const updateSensor = await SensorRepo.findOneAndUpdate({_id: sensorInput._id}, data, {new: true}).catch((err:Error) => {
                console.log(err.message)
            })
            return updateSensor as unknown as Sensor
        },
        deleteSensor: async (_id:string):Promise<Sensor> =>{
            const deleteSensor = await SensorRepo.findOneAndDelete({_id: _id}).catch((err:Error) => {
                console.log(err.message)
            })
