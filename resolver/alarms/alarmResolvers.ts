...
import type { AlarmCreate, AlarmUpdate, SensorID, SensorInput } from "./alarms.d.ts";
...
export const alarmResolvers = {
    ...
    Mutation: {
        ...
        updateAlarm: async (alarmInput: AlarmUpdate) => {
            ...
            const updateAlarm = await AlarmRepo.findOneAndUpdate({_id: alarmInput._id}, data, {new: true}).catch((err:Error) => {
                console.log(err.message)
            })
            return updateAlarm as unknown as Alarm
        },
        deleteAlarms: async ( sensor:SensorID ) => {
            const deleteAlarms = await AlarmRepo.deleteMany({sensor: sensor.sensor})
