...
export const createEntry = async (data: Buffer) => {
    ...
    let currSensor: Sensor = await sensorsResolvers.Query.getSensorByAddressAndId(null,
        { address: parsedData.address, sensor_id: parsedData.id }
    );
    ...
    if (currentDate - lastAdded <= 60000 && prevSensor.status === parsedData.status) {
        return;
    }
    ...
    const { status: sensorStatus, down_limit, up_limit, type: sensorType } = currSensor;
    ...
    const alarm = await alarmResolvers.Query.getAlarmByAddressAndId(null,
        { address: parsedData.address, sensor: parsedData.id }
    );
...
function getAlarmReason(value: number, sensor: Sensor): string {
    switch (sensor.type) {
        case "Gauge":
            return value > sensor.up_limit ? 'Υψηλή Θερμοκρασία' : 'Χαμηλή Θερμοκρασία';
        case "Digital":
...
function shouldSkip(prev: IData | undefined, next: IData): boolean {
    ...
    const valuesMatch = Number(prev.value.toFixed(1)) === Number(next.value.toFixed(1));
