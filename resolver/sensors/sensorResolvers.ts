...
import type { SensorBatchInput, SensorCreate, SensorUpdate, AverageData } from "./sensors.d";
...
export const sensorsResolvers = {
    Query: {
        ...
        getSensorEntriesLast24Hours: async (_: unknown, { sensors }: { sensors: { address: string; sensor_id: number }[] }) => {
            ...
            return Object.entries(averageResults).map(([sensorId, { averages, counts, address }]) => {
